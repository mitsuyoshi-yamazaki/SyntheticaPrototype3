...
describe("calculateTotalSeparationForce", () => {
  ...
  test("衝突していないオブジェクトは無視される", () => {
    ...
    const totalForce = calculateTotalSeparationForce(obj, collidingObjects, worldWidth, worldHeight)

    // colliding オブジェクトからの反発力のみ
    const singleForce = calculateSeparationForce(obj, colliding, worldWidth, worldHeight)
  ...
  test("空の配列では反発力0", () => {
    ...
    const totalForce = calculateTotalSeparationForce(obj, [], worldWidth, worldHeight)
  ...
  test("周囲から押しつぶされる状況", () => {
    ...
    const totalForce = calculateTotalSeparationForce(center, surrounding, worldWidth, worldHeight)
  ...
  test("カスタムパラメータの適用", () => {
    ...
    const totalForce = calculateTotalSeparationForce(obj, [other], worldWidth, worldHeight, params)
