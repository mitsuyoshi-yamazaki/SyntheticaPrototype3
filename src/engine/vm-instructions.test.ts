...
describe("0xA0 LOAD_ABS", () => {
  ...
  test("LOAD_ABS実行 - 絶対アドレスから8bit読み込み", () => {
    vm.setRegister("A", 0xffff)
    vm.setRegister("B", 0x1111)
    vm.setRegister("C", 0x2222)
    vm.setRegister("D", 0x3333)
    ...
    expect(vm.readMemory8(0x80)).toBe(0x42)
    ...
    expectVMState(vm, {
      pc: 4,
      sp: 0xff,
      registerA: 0x42, // 上位8bitは0、下位8bitに読み込んだ値
      registerB: 0x1111,
      registerC: 0x2222,
      registerD: 0x3333,
      carryFlag: false,
...
describe("0xA1 STORE_ABS", () => {
  ...
  test("STORE_ABS実行 - 絶対アドレスへ8bit書き込み", () => {
    vm.setRegister("A", 0x1234)
    vm.setRegister("B", 0x5678)
    vm.setRegister("C", 0x9abc)
    vm.setRegister("D", 0xdef0)
    ...
    expect(vm.readMemory8(0x90)).toBe(0x00)
    ...
    expect(vm.readMemory8(0x90)).toBe(0x34) // Aの下位8bitが書き込まれる
  })
})
...
describe("0xA2 LOAD_ABS_W", () => {
  ...
  test("LOAD_ABS_W実行 - 絶対アドレスから16bit読み込み", () => {
    vm.setRegister("A", 0xffff)
    vm.setRegister("B", 0x1111)
    vm.setRegister("C", 0x2222)
    vm.setRegister("D", 0x3333)
    ...
    expect(vm.readMemory8(0x80)).toBe(0x34)
    expect(vm.readMemory8(0x81)).toBe(0x12)
    ...
    expectVMState(vm, {
      pc: 4,
      sp: 0xff,
      registerA: 0x1234, // 16bit値が読み込まれる
      registerB: 0x1111,
      registerC: 0x2222,
      registerD: 0x3333,
      carryFlag: false,
...
describe("0xA3 STORE_ABS_W", () => {
  ...
  test("STORE_ABS_W実行 - 絶対アドレスへ16bit書き込み", () => {
    vm.setRegister("A", 0x5678)
    vm.setRegister("B", 0x1111)
    vm.setRegister("C", 0x2222)
    vm.setRegister("D", 0x3333)
    ...
    expect(vm.readMemory8(0x90)).toBe(0x00)
    expect(vm.readMemory8(0x91)).toBe(0x00)
    ...
    expect(vm.readMemory8(0x90)).toBe(0x78) // 下位バイト
    expect(vm.readMemory8(0x91)).toBe(0x56) // 上位バイト（リトルエンディアン）
  })
})
...
describe("0xB0 JMP_IND", () => {
  ...
  test("JMP_IND実行 - レジスタAの値へジャンプ", () => {
    vm.setRegister("A", 0x0050)
    vm.setRegister("B", 0x1111)
    vm.setRegister("C", 0x2222)
    vm.setRegister("D", 0x3333)
...
describe("0xC0 MUL_AB", () => {
  ...
  test("MUL_AB実行 - 16bit乗算", () => {
    ...
    expectVMState(vm, {
      pc: 5,
      sp: 0xff,
      registerA: 0x0200, // 0x10 * 0x20 = 0x200
      registerB: 0x0020,
      registerC: 0x1111,
      registerD: 0x2222,
      carryFlag: false,
  ...
  test("MUL_AB実行 - オーバーフロー時は下位16bitのみ保持", () => {
    ...
    expectVMState(vm, {
      pc: 5,
      sp: 0xff,
      registerA: 0x0000, // 0x1000 * 0x100 = 0x100000 → 下位16bit = 0x0000
      registerB: 0x0100,
      registerC: 0x3333,
      registerD: 0x4444,
      carryFlag: false,
      zeroFlag: true, // 結果が0なのでゼロフラグセット
    })
  })
})
...
describe("0xC1 DIV_AB", () => {
  ...
  test("DIV_AB実行 - 16bit除算とモジュロ", () => {
    ...
    expectVMState(vm, {
      pc: 5,
      sp: 0xff,
      registerA: 0x000e, // 100 / 7 = 14
      registerB: 0x0002, // 100 % 7 = 2
      registerC: 0x1111,
      registerD: 0x2222,
      carryFlag: false,
      zeroFlag: false,
...
describe("0xC2 SHL", () => {
  ...
  test("SHL実行 - 論理左シフト", () => {
    ...
    expect(result.success).toBe(true)
    expect(result.error).toBeUndefined()
    expect(result.cycles).toBe(4) // ビット操作は4サイクル

    // 実行後の状態を検証
    expectVMState(vm, {
      pc: 5,
      sp: 0xff,
      registerA: 0x0010, // 0x0001 << 4 = 0x0010
      registerB: 0x0004,
      registerC: 0x1111,
      registerD: 0x2222,
      carryFlag: false,
  ...
  test("SHL実行 - 最大15ビットシフト", () => {
    ...
    expectVMState(vm, {
      pc: 5,
      sp: 0xff,
      registerA: 0x8000, // 0x0001 << 15 = 0x8000
      registerB: 0x100f,
      registerC: 0x3333,
      registerD: 0x4444,
      carryFlag: false,
...
describe("0xC3 SHR", () => {
  ...
  test("SHR実行 - 論理右シフト", () => {
    ...
    expectVMState(vm, {
      pc: 5,
      sp: 0xff,
      registerA: 0x0100, // 0x1000 >> 4 = 0x0100
      registerB: 0x0004,
      registerC: 0x1111,
      registerD: 0x2222,
      carryFlag: false,
...
describe("0xC4 SAR", () => {
  ...
  test("SAR実行 - 算術右シフト（符号ビット保持）", () => {
    vm.setRegister("A", 0x8000) // 最上位ビットが1（負の数）
    vm.setRegister("B", 0x0004) // 4ビット右シフト
    vm.setRegister("C", 0x1111)
    vm.setRegister("D", 0x2222)
...
describe("0xC5 CMOV_Z", () => {
  ...
  test("CMOV_Z実行 - ゼロフラグセット時に移動", () => {
    ...
    expect(result.success).toBe(true)
    expect(result.error).toBeUndefined()
    expect(result.cycles).toBe(3) // 条件付き移動は3サイクル

    // 実行後の状態を検証
    expectVMState(vm, {
      pc: 5,
      sp: 0xff,
      registerA: 0x2222, // BからAへコピー
      registerB: 0x2222,
      registerC: 0x3333,
      registerD: 0x4444,
      carryFlag: false,
