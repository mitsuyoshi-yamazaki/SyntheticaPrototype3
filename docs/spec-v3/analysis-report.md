...
- **COMPUTER生成時のメモリ書き込み機構の最終仕様決定**（案B暫定採用、Synthetica Script仕様後に再考）
...
**改善案**:
- 命令を8bit単位に変更（例: 8bitオペコード + 8bitオペランド）
- または、メモリを32bit単位でアドレッシングに変更

### 9.2 エネルギーソースの配置・管理仕様の欠如 🔴 **重要**
...
**必要な仕様**:
- 熱ダメージ閾値（例: 100度以上で1tick毎に1Eダメージ）
- ユニット状態による耐熱性の変化
...
**必要な仕様**:
- 形状の定義（円形？矩形？）
- 反発力 = k * 重なり量 * 質量比 のような具体的な式

### 9.5 ASSEMBLERの生産中ユニットのエネルギーコスト 🔴 **重要**

**問題**: `game-world-requirement.md:234` で「かかるエネルギーコストは TBD」
- 自己複製コスト計算に必須
- ゲームバランスの根幹

**提案**: 生産中ユニット生成に必要なエネルギー = 完成ユニットの構成エネルギーの10%
