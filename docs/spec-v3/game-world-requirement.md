...
- 処理順：
  1. **ユニットアクションフェーズ**
     1-1. ASSEMBLER/DISASSEMBLER動作（前tickで予約されていた動作）
     1-2. HULL動作（回収/放出/マージ）（前tickで予約されていた動作）
...
- 予約時と実行時の状況変化：
  - エネルギー不足時の動作はアクション種別に依存（「何も起きない」または「不完全実行/全て熱変換」）
...
- 仕様：
  ...
  - 拡張
    ...
    - マージ後のHULLの容量はマージ前の容量の合計値であり、マージ前にそれぞれのHULLに格納されていたゲームオブジェクトと、固定されていたユニットはすべてマージ後のHULLへ移る
- 回路仕様：
...
- 生成仕様：
  - TODO:
    - ユニット構成エネルギーおよびユニット生産エネルギーを決める。それぞれのエネルギーはassemble powerが大きいほど大きくなる。また、assemble powerと比例してASSEMBLERの占める容積は大きくなる
- 分解仕様：
