...
- **0x1F: PUSH_A** - Aレジスタをスタックにプッシュ
- **0x20: PUSH_B** - Bレジスタをスタックにプッシュ
- **0x21: PUSH_C** - Cレジスタをスタックにプッシュ
- **0x22: PUSH_D** - Dレジスタをスタックにプッシュ
- **0x23-0x2D: 未定義**
- **0x2E: POP_A** - スタックからAレジスタにポップ
- **0x2F: POP_B** - スタックからBレジスタにポップ
- **0x30: POP_C** - スタックからCレジスタにポップ
- **0x31: POP_D** - スタックからDレジスタにポップ
- **0x32-0x3F: 未定義**
...
##### 動的ユニット操作命令
- **0x9B: UNIT_MEM_WRITE_DYN** - レジスタ指定アドレスへのユニットメモリ書き込み
  - 第2バイト: ユニット種別とインデックス（上位4bit:種別、下位4bit:インデックス）
  - 第3バイト: アドレス指定レジスタ（0=A, 1=B, 2=C, 3=D）
  - 第4バイト: 未使用（0x00）
  - 指定レジスタの値をユニット内メモリアドレスとして使用
  - Aレジスタの下位8bitを書き込み
...
- 1バイト命令: 1E
  ...
  - スタック操作命令（PUSH_*, POP_*）: 2E（メモリアクセスを含むため）
- 3バイト命令: 3E
...
- 4バイト命令: 4E
  ...
  - 動的ユニット操作（UNIT_MEM_WRITE_DYN）: 4E + 10E（ユニット操作）
- 5バイト命令: 5E
  - 即値ロード（LOAD_IMM）: 5E
  - ビットシフト命令（SHL, SHR, SAR）: 4E（ビット操作のため）
  - 条件付き移動命令（CMOV_*）: 3E（分岐を伴わないため）
- テンプレートマッチング:
  - 基本コスト: 3E
...
```
; Cレジスタの値をアドレスとしてユニットメモリに書き込み
0x00: LOAD_IMM    ; A = 書き込む値
0x01: 0x42
0x02: 0x00
0x03: LOAD_IMM    ; C = アドレス
0x04: 0x10
0x05: 0x00
0x06: MOV_AC
0x07: UNIT_MEM_WRITE_DYN
0x08: 0xC1        ; COMPUTER[1]
0x09: 0x02        ; Cレジスタのアドレスへ
0x0A: 0x00        ; 未使用
```
