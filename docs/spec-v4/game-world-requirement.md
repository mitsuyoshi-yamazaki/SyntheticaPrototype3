...
### v4仕様の概要とゴール
- COMPUTERの実行するSynthetica Scriptの実装が想定より負担が大きかったため、また本プロジェクトはプロトタイピングであるため、JS Interpreterで実現するように変更する
...
- このゲーム空間上に存在するゲームオブジェクトは、全て1セル分の大きさを持つ
- ゲームオブジェクトの種類によって、セルに複数同時に存在できるものと、そうでないものがある
...
- ゲームオブジェクトは以下のプロパティをもつ
  - 位置[Position]
    - 位置は離散的数値で表される。これは「そのゲームオブジェクトが存在するセルの座標」と同じである
  - 大きさ、形状は、ゲーム仕様上は全て同一で、セルの大きさ、形状と等しい。しかし、アプリケーションの見た目としてはオブジェクトの種別ごとに異なる大きさ、形状を取る
  - セルに対しての移動になるので、速度や加速度の概念はない
  - 重量[Weight]
    - 重量は、そのゲームオブジェクトにどれだけ力がかかったら隣セルへ移動するかに影響する数値である
- 仕様
  - 衝突[Collision]
    - ゲームオブジェクト同士は衝突せず、ひとつのセルに対して複数のゲームオブジェクトが存在できる
    - セルに侵入不能な地形[Terrain]が設定されている場合は、どのゲームオブジェクトもそのセルには侵入できない
  - 隣接[Adjacent]
    - ふたつのオブジェクトが"隣接"している状態とは以下のいずれかの状態である
      - 両方のオブジェクトが同じセルに存在する場合
      - それぞれのオブジェクトの存在するセルが隣り合っている（ムーア近傍）場合
      - *TODO: HULLの内部のオブジェクトの隣接条件*
...
- t tickの処理順：
  1. **ユニットアクションフェーズ**
    - ユニットの操作対象となるのはt-1 tickの世界の状態である
     1-1. それぞれのユニットに対して、予約されたアクションのためのエネルギーを確保する（確保順序：`energy-action-order.md`参照）（オブジェクトの移動に関する操作は、操作が予約された状態となり、 2. 物理演算フェーズ で移動を行う）
...
- アクション予約制：
  - 全てのユニットの操作は「次のtickで行うアクションを予約する」形で指示される
  - COMPUTERのソフトウェア実行時に各ユニットへのアクション予約が行われる
...
- 熱ダメージ仕様：
  - 熱により受けるダメージは各オブジェクト種別、各ユニット種別により異なる *TODO*
...
- 仕様：
  ...
  - ユニット分解[Disassemble]
    - ユニットは環境ダメージを受けることにより構成エネルギーを失う（= ダメージを負う）
      - 軽微なダメージはASSEMBLERにより、失ったエネルギーを補充することで修復が可能である
      - 構成エネルギーが0になったユニットは（エネルギーに変化することで）消滅する
    - ユニットは消滅するまで、その機能を失わない
  - 相互に接続しているユニットは、COMPUTERのソフトウェアからそのユニットへアクセスを可能にする回路[Circuit]として接続することができる（ユニットの「回路仕様」に記載）
  - ユニットはHULLの外側または内側に接続[Attach]することができる。HULLも例外ではない
  - ユニットの状態：
    - **HULL固定状態**: HULLに物理的に固定されている
    - **HULL格納状態**: HULLの内部空間に格納されている
    - **自由状態**: 座標を持ってゲーム世界を漂っている
  - ユニット結合構造の表現：
    - HULL中心の管理方式を採用
    - 各HULLが固定ユニット(attachedUnits)と格納オブジェクト(containedObjects)を管理
  - 隣接・ダメージルール：
    - HULL固定ユニットは、そのHULLの隣接状態を継承
    - 熱ダメージは固定/格納状態によらず、そのセルに存在する全ユニット・全オブジェクトが受ける
  - 基本的な構想として、ユニットが何かの行動を起こそうとして失敗する場合、ソフトウェア的なエラーは返さず、実行はするが意図した結果が得られない、という現象として実現する
- 回路仕様：
  - 回路的に接続しているユニットへはソフトウェア上で情報アクセスおよび指示ができる
  - 本仕様においては同一HULLに固定された全ユニットが相互にアクセス可能
  - 接続数の上限なし
- COMPUTERから各ユニットにアクセスし、ユニット状態の読み取りやユニット機能の実行を行うために、ユニットを指定したうえでユニットの操作APIを利用する
- ユニットには以下の種別がある
  - HULL
  - ASSEMBLER
  - COMPUTER
